...
// password in an owner-only file rather than a Plex token - both said out loud below. What the
// DB8 line added for BOTH flavours: the reporting answers and identifiers share one versioned
// record in a private database kind, and signing out commits one durable tombstone before a new
...
const PRIVACY_CURRENT: &str = concat!(
    ...
    "ON THIS TELEVISION\n\n",
    "butaca stores the address of your Jellyfin server, your username and your password (in a file only the app can read), a device identifier, your Home library choices, your recent searches, your playback quality preference, local diagnostic logs and a crash-log watermark. It also stores any report waiting to be sent. It keeps no bookmark of its own for where you stopped watching: playback position is held by your Jellyfin server.\n\n",
    "Your answers to the two optional-reporting questions, and the random Crash report ID and Analytics ID if they exist, share one versioned record in a private database kind owned by butaca's packaged storage service. The database holds fixed metadata and an opaque typed document. Private DB8 is not encryption and does not protect data from root access. Your sign-in itself - the server address, username and password - is kept in an owner-only file outside the application's own directory, so reinstalling does not sign you out; older copies of that file are migration inputs and are removed only after a verified import.\n\n",
    "OPTIONAL CRASH REPORTS\n\n",
    "If enabled, technical crash details are sent to Sentry in Germany. They can include the signal, code addresses, thread information and device compatibility details. Each report carries a random Crash report ID created on this television when you turned crash reports on, so that repeated crashes under one Crash report ID are counted once rather than once each. It is not derived from your Jellyfin account, your television or anything about you, and it is never sent with product analytics. Settings shows it as your Crash report ID. A storage report found before you answer the crash-reports question waits in memory and is discarded after No or sign-out; closing the app loses that queued copy, and it is never sent after No.\n\n",
    "OPTIONAL PRODUCT ANALYTICS\n\n",
    "If enabled, screen and feature events and broad sign-in and playback outcomes are sent to PostHog in Germany with a random Analytics ID created when you turned product analytics on. Settings shows that identifier as your Analytics ID. Crash reports and analytics are independent and off until you enable them.\n\n",
    "NEVER INCLUDED\n\n",
    ...
    "RETENTION\n\n",
    "Signing out immediately closes server access and both reporting gates, removes your sign-in file, then commits one durable tombstone for the reporting record before a new sign-in is permitted. Your answers to the two optional-reporting questions, and the Crash report ID and Analytics ID if they exist, belong to that sign-in: signing out removes them with it, and whoever signs in next is asked afresh. A report waiting to be sent is deleted once it is sent; signing out, or Delete all local data, is a harder stop: it destroys everything queued at once. Turning a reporting category off immediately closes its gate, deletes its identifier and queues removal of that category's reports; one request already transmitted cannot be recalled. Event and stderr logs start fresh on each launch; the crash log is append-only across launches until Delete all local data queues its removal. A report that has already been sent is held by the service that received it, under that service's own retention schedule; write to the contact below to ask what those periods currently are.\n\n",
    ...
    "UNINSTALLING\n\n",
    "Removing butaca removes the application and its private database kind. Your sign-in file and any legacy external migration files can survive uninstall because webOS gives an application no way to run code as it is removed. Use Delete all local data and resolve any cleanup warning BEFORE uninstalling if you want nothing of butaca left on this television.\n\n",
...
// Historical field-level fixture from the 0.6 line. Production displays `PRIVACY_CURRENT`; this
// assertion target is retained only to guard the already-published report vocabulary.
...
mod tests {
    ...
    /// claim it completed when the receipt says otherwise. The quarantined-file machinery is
    /// Plex-session-specific; the Jellyfin flavour's credential is its own config file, so this
    /// vocabulary test stays on the Plex build.
    #[cfg(not(feature = "jellyfin"))]
    #[test]
